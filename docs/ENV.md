...
| Переменная        | По умолчанию | Описание                                                           |
|-------------------|--------------|----------------------------------------------------------------------|
| `OPENAI_MODEL`    | `gpt-5.1`    | Имя модели OpenAI. Можно указывать `gpt-5-mini`, чтобы включить reasoning-модель через Responses API. |
...
| `OPENAI_REASONING_EFFORT` | —    | Уровень `reasoning.effort` для reasoning-моделей (`low`/`medium`/`high`). Опциональный параметр. |


Дополнительно в коде заданы лимиты: `OPENAI_MAX_TOKENS=1800`, `MAX_REPLY_CHARS=3500`, `HISTORY_MAX_TURNS=30`. Изменяются
...
| `MAX_REPLY_CHARS`            | `3_500`                | Жёсткий максимум символов в ответе, отправляемом пользователю.      |
