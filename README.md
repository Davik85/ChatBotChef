...
- **Интерфейс с Telegram** — модуль `app.telegram` реализует long-polling, нормализацию апдейтов, защиту от вложений и
...
- **Бизнес-логика** — модули `app.logic` реализуют лимиты, подсказки, историю диалога, напоминания об окончании подписки,
  сервис работы с пользователями (`app.logic.user.UserService`) и статистику (`app.logic.stats.StatsService`).
...
## Админ-панель, статистика и рассылки

- Список администраторов задаётся через переменную окружения `ADMIN_IDS` и читается в `AppConfig.adminIds`. Администраторы
  не попадают под ограничения бесплатного тарифа, для них не ведётся счётчик `usage_counters`, а доступ к режимам всегда
  открыт.
- Команда `/admin` открывает панель с кнопками статистики, подготовки рассылки и выдачи премиума. Панель автоматически
  сбрасывается после команды `/start` или любого пользовательского режима.
- `/premiumstatus <tgId>` показывает информацию о пользователе: дату регистрации и последнего контакта, статус блокировки,
  наличие премиума и количество сообщений. `/grantpremium <tgId> <days>` продлевает или выдаёт доступ на указанное число
  дней; все операции фиксируются в `admin_audit`.
- `StatsService` собирает метрики без сложных JOIN-ов: общее число пользователей (`users.total`), активные установки
  (`total - blocked`), премиум-подписчиков (`premium_users.until_ts > now`), заблокировавших бота (`users.blocked`/`blocked_ts`)
  и количество активных за 7 дней по полю `users.last_seen`. Эти значения отображаются в панели и используются в
  рассылках.
- Рассылки берут аудиторию только из активных пользователей (`users.blocked = 0`, `blocked_ts <= 0`). При ошибках отправки
  «bot was blocked…» пользователь помечается заблокированным, событие логируется, но рассылка не прерывается. В ответ
  администратор получает статистику: сколько сообщений доставлено, сколько дало ошибку блокировки и сколько завершилось
  другими ошибками.
- Таблица `users` обновляется на каждом апдейте: для каждого входящего/исходящего события сохраняется `last_seen`, а при
  успешной отправке сбрасывается признак блокировки. Ошибки доставки, наоборот, фиксируют `blocked_ts` и переводят
  пользователя в состояние `blocked`.
